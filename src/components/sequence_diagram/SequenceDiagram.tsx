...
export default function SequenceDiagram() {
    ...
    // const maxNumEvents = d3.max(activeFiles.map((file) => file.events.length)) || 0;
    ...
    // const timeStamps = allEvents.map(event => event.time);

    const xScale =  d3.scalePoint().domain(activeFiles.map(file => file.name)).range([0, innerWidth]);
    // const xScale = d3.scaleLinear().domain([0, activeFiles.length - 1]).range([0, innerWidth]);
    ...
    const diagram = (
        <svg width={width} height={height} className="bg-white border border-gray-200 rounded-lg shadow-inner dark:bg-gray-700 dark:border-gray-700">
            <g transform={`translate(${margin.left}, ${margin.top})`}>
                {activeFiles.map(file => <Axis key={file.name} xPos={xScale(file.name)! + eventBlockSize / 2} yPos={0} height={innerHeight} />)}
